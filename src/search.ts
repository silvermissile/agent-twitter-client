...
async function getSearchTimeline(
  ...
  const res = await requestApi<SearchTimeline>(
    `https://x.com/i/api/graphql/ML-n2SfAxx5S_9QMqNejbg/SearchTimeline?${params.toString()}`,
    auth,
  );
...
export async function fetchQuotedTweetsPage(
  ...
  const url = `https://x.com/i/api/graphql/ML-n2SfAxx5S_9QMqNejbg/SearchTimeline?${params.toString()}`;
